...
namespace AtomicSeller.Helpers.Razor.HtmlExtensions
{
    public static class SubmitWrapExtension
    {
        ...
        public static HtmlString SubmitWrap(this IHtmlHelper htmlHelper, string text, string icon = DefaultIcon,
            Position position = DefaultPosition, IconPosition iconPosition = DefaultIconPosition)
        {
            ...
            switch (position)
            {
                case Position.Inline:
                    return new HtmlString(getButton(buttonClasses, text, icon, iconPosition));
                case Position.PullRight:
                    return new HtmlString(getButton(buttonClasses + " pull-right", text, icon, iconPosition) + HtmlConsts.Snippets.ClearFix);
